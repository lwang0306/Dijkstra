...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (args.length < 4) {
			System.out
					.println("Usage: java Main [name_of_data_file] [label_of_start_vertex] [label_of_dest_vertex_1] [label_of_dest_vertex_2]");
			System.exit(0);
		}
		...
		System.out.println("Time to find shortest path (first time) from "
				+ startLabel + " to " + destLabel1 + ": " + (end - start)
				+ "ms");
		...
		System.out.println("Time to find shortest path (second time) from "
				+ startLabel + " to " + destLabel1 + ": " + (end - start)
				+ "ms");

		// calculate the cost of the shortest path from the start to the first
		// destination
		start = System.currentTimeMillis();
		double cost = g.shortestPathCost(startLabel, destLabel1);
		end = System.currentTimeMillis();
		System.out.println("Time to find cost of shortest path from "
				+ startLabel + " to " + destLabel1 + ": " + (end - start)
				+ "ms");
		...
		System.out.println("Time to find shortest path from " + startLabel
				+ " to " + destLabel2 + ": " + (end - start) + "ms");
	...
	public static WeightedGraph buildGraph(String fileName) {
		...
		try {
			...
			// create the graph with the capacity equal to the number of
			// vertices
			...
			for (int i = 0; i < numVertices; i++) {
				if (in.hasNext() == false) {
					...
					throw new Exception(
							"Error reading input file: seems like number of vertices is wrong");
				}
				String label = in.next();
				int x = in.nextInt();
				int y = in.nextInt();
				g.addVertex(label, x, y);
				// System.out.println("Added vertex " + label + " at (" + x +
				// ", " + y + ")");
			}
			...
			for (int i = 0; i < numEdges; i++) {
				if (in.hasNext() == false) {
					// uh-oh, someone lied to us
					throw new Exception(
							"Error reading input file: seems like number of edges is wrong");
				}
				String a = in.next();
				String b = in.next();
